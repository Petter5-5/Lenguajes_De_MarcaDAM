// Array con las rutas de las imágenes de memes
const imagenesMemeCabecera = [
    'imagenes/meme01.png',
    'imagenes/meme02.png',
    'imagenes/meme03.png',
    'imagenes/meme04.png',
...
const color_Superior = document.getElementById('color_Superior');
const texto_Superiro = document.getElementById('texto_Superiro');
const barra_Inferior = document.getElementById('barra_Inferior');
const color_Inferior = document.getElementById('color_Inferior');
const texto_inferior = document.getElementById('texto_inferior');
const descargar_Meme = document.getElementById('descargar_Meme');
const seccion_Meme = document.getElementById('seccion_Meme');
const labelSuperiro = document.getElementById('labelSuperiro');
const labelInferior = document.getElementById('labelInferior');
...
function cargarMemesAleatorios() {
    const memeIzq = document.getElementById('meme-izq');
    const memeDer = document.getElementById('meme-der');
    
    // Generar dos índices aleatorios diferentes
    let indice1 = Math.floor(Math.random() * imagenesMemeCabecera.length);
    let indice2;
    
    do {
        indice2 = Math.floor(Math.random() * imagenesMemeCabecera.length);
    } while (indice2 === indice1);
    
    memeIzq.src = imagenesMemeCabecera[indice1];
    memeDer.src = imagenesMemeCabecera[indice2];
}

// Cargar imagen del meme
file.addEventListener('change', function(e) {
    ...
    lector.onload = function(evento) {
        imagenBase.src = evento.target.result;
        seccion_Meme.classList.remove('invisible');
        seccion_Meme.classList.add('seccion_Meme');
    };
    
    lector.readAsDataURL(archivo);
});

barra_Superior.addEventListener('input', function() {
    labelSuperiro.textContent = `Tamaño texto superior (${barra_Superior.value} px)`;
    superior.style.fontSize = barra_Superior.value + 'px';
});

color_Superior.addEventListener('input', function() {
    superior.style.color = color_Superior.value;
});

texto_Superiro.addEventListener('input', function() {
    superior.textContent = texto_Superiro.value;
});

barra_Inferior.addEventListener('input', function() {
    labelInferior.textContent = `Tamaño texto inferior (${barra_Inferior.value} px)`;
    inferior.style.fontSize = barra_Inferior.value + 'px';
});

color_Inferior.addEventListener('input', function() {
    inferior.style.color = color_Inferior.value;
});

texto_inferior.addEventListener('input', function() {
    inferior.textContent = texto_inferior.value;
});

// Descargar meme - CÓDIGO PROPORCIONADO POR EL PROFESOR
descargar_Meme.addEventListener('click', function() {
    if (!imagenBase.src) {
        ...
        imagen_meme_final.onload = async () => {
            ...
            dibujarTextoDelMeme(superior, "top");
            // Dibujo el texto de abajo del meme
            dibujarTextoDelMeme(inferior, "bottom");
...
// Cargar memes aleatorios al cargar la página
window.addEventListener('load', cargarMemesAleatorios);